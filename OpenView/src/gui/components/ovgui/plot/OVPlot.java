...
public class OVPlot extends OVNodeComponent implements SlotListener {
	...
	private void initPlot() {
		try {
			...
			switch (type) {
			case STAIRCASE:
				plot_ = new StaircasePlot(name, color, new double[1][1]);
				break;
			case SCATTER:
				plot_ = new ScatterPlot(name, color, new double[1][1]);
				break;
			case HISTOGRAM:
				plot_ = new HistogramPlot2D(name, color, new double[1][1], 10);
				break;
			case CLOUD:
				plot_ = new CloudPlot2D(name, color, new double[1][1], 3, 3);
				break;
			case BAR:
				plot_ = new BarPlot(name, color, new double[1][1]);
			case LINE:
			default:
				plot_ = new LinePlot(name, color, new double[1][1]);
				break;
			}
