...
public class Round extends AbstractFunction {
	...
	private static final String description = "Returns the cosine of a value; the returned angle is in the range 0.0 through pi.";

	public Round() {
		super("round", "Round", description, 1);
	}
	...
	public Value evaluate(Value... arguments) throws Exception {
		...
		if (arguments.length == 1) {
			...
			if (value.getType().isNumeric()) {
				return new Value(new Double(Math.round(value.getDouble())));
			}
